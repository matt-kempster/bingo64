...
void one_triangle_visual(s32 x, s32 z, Vtx *verts) {
    ...
    surf = gMarioState->floor;
    if (surf == NULL) {
        return;
    }
    make_vertex(verts, gVisualSurfaceCount, surf->vertex1[0], surf->vertex1[1], surf->vertex1[2], 0, 0,
                col[0], col[1], col[2], 0x80);
    make_vertex(verts, gVisualSurfaceCount + 1, surf->vertex2[0], surf->vertex2[1], surf->vertex2[2], 0,
                0, col[0], col[1], col[2], 0x80);
    make_vertex(verts, gVisualSurfaceCount + 2, surf->vertex3[0], surf->vertex3[1], surf->vertex3[2], 0,
                0, col[0], col[1], col[2], 0x80);
...
void visual_surface_display(Vtx *verts) {
    ...
    while (vts > 0) {
        if (count == 30) {
            ntx = MIN(30, vts);
            gSPVertex(gDisplayListHead++, VIRTUAL_TO_PHYSICAL(verts + ((gVisualSurfaceCount) -vts)),
                      ntx, 0);
            count = 0;
            vtl = ntx;
        }
        ...
        if (vtl >= 6) {
            gSP2Triangles(gDisplayListHead++, count + 0, count + 1, count + 2, 0, count + 3, count + 4,
                          count + 5, 0);
            vts -= 6;
            vtl -= 6;
            count += 6;
...
void visual_surface_loop(void) {
    ...
    verts = alloc_display_list((iterate_surface_count(gMarioState->pos[0], gMarioState->pos[2]) * 3)
                               * sizeof(Vtx));
...
void one_triangle_loop(void) {
    ...
    mtx = alloc_display_list(sizeof(Mtx));
    verts = alloc_display_list(3 * sizeof(Vtx));
...
#define DBG_BOX_VTX_CYL(i, x, y, z)                                                                    \
    make_vertex(debugCylinderVtx, i, x, y, z, 0, 0, 0xFF, 0xFF, 0xFF, 0xFF)
...
static void render_box(int index, Vtx *vbox, Vtx *vcylinder) {
    ...
    guScale(scale, (f32) box->bounds[0] * 0.01f, (f32) box->bounds[1] * 0.01f,
    ...
    gDPSetEnvColor(gDisplayListHead++, (color >> 16) & 0xFF, (color >> 8) & 0xFF, (color) &0xFF,
                   (color >> 24) & 0xFF);
...
void render_debug_boxes(void) {
    ...
    for (i = 0; i < (CYLINDER_VERTS); i++) {
        DBG_BOX_VTX_CYL(1 + i, 0 + (100 * coss((0xFFFF / ((CYLINDER_VERTS) / 2)) * (i / 2))),
                        0 + (100 * (i % 2)),
