...
void bhv_yoshi_init(void) {
    ...
    // bingo64: the full-game unlock stamps all 120 stars into the save file so
    // every door opens, which would put Yoshi on the roof from the start. Gate
    // him on the stars actually collected this race instead.
    s32 starCount = gBingoFullGameUnlocked
        ? bingo_get_star_count()
        : save_file_get_total_star_count(gCurrSaveFileNum - 1, COURSE_MIN - 1, COURSE_MAX - 1);

    if (starCount < 120 || sYoshiDead == TRUE) {
