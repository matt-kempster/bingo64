...
void bhv_1up_hidden_trigger_loop(void) {
    ...
    if (obj_check_if_collided_with_object(o, gMarioObject) == 1) {
        ...
        if (sp1C == NULL) {
            sp1C = cur_obj_nearest_object_with_behavior(bhv1upGreenDemon);
        }
        if (sp1C != NULL) {
            sp1C->oHidden1UpNumTouchedTriggers++;
        }
...
void bhv_1up_green_demon_loop(void) {
    if (gBingoStarSelected == BINGO_MODIFIER_GREEN_DEMON) {
        ...
        if (o->oAction == 1 && obj_check_if_collided_with_object(o, gMarioObject) == 1) {
